...
public class PlayerMove : MonoBehaviour
{
    ...
    void FixedUpdate() //for physics computations
    {
        Move();
    }
    ...
    void Move()
    {
        float h = Input.GetAxisRaw("Horizontal"); //for D and right arrow. For ex: when we press D the value of h will go from 0 to 1 immediately. If it was just getaxis it would increment from 0 to 1 slowly.
        float v = Input.GetAxisRaw("Vertical");

        if(h > 0) //means we are going to right
        {
        ...
        if(h > 0) //making sure the character is facing the right direction when moving
        {
            tempScale.x = -1f;
        } else if(h < 0)
        {
